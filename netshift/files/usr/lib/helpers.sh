...
extract_subscription_metadata_from_errfile() {
    local errfile="$1"
    local outfile="$2"
    ...
    awk '
    BEGIN {
        upload = 0; download = 0; total = 0; expire = 0; refill = 0; title = "";
        found = 0;
    }
    {
        line = $0;
        ...
        if (tolower(line) ~ /^subscription-userinfo:/) {
            found = 1;
            sub(/^[^:]+:[ \t]*/, "", line);
            n = split(line, pairs, /[ \t]*;[ \t]*/);
            for (i = 1; i <= n; i++) {
        ...
        else if (tolower(line) ~ /^profile-title:/) {
            found = 1;
            sub(/^[^:]+:[ \t]*/, "", line);
            title = line;
        }
        else if (tolower(line) ~ /^subscription-refill-date:/) {
            found = 1;
            sub(/^[^:]+:[ \t]*/, "", line);
            refill = line;
        }
    ...
    END {
        if (found) {
            gsub(/\\/, "\\\\", title);
            gsub(/"/, "\\\"", title);
            printf "{\"upload\":%s,\"download\":%s,\"total\":%s,\"expire\":%s,\"refill\":%s,\"title\":\"%s\"}\n",
                (upload ~ /^[0-9]+$/ ? upload : 0),
...
download_subscription() {
    ...
    for attempt in $(seq 1 "$retries"); do
        ...
        if [ "$rc" -eq 0 ] && [ -s "$tmpfile" ]; then
            ...
            extract_subscription_metadata_from_errfile "$errfile" "${filepath}.meta.json"
            rm -f "$errfile"
            return 0
        fi

...
sing_box_cf_add_proxy_outbound() {
    ...
    hysteria2 | hy2 | hysteria)
        ...
        host=$(url_get_host "$url")
        port="$(url_get_port "$url")"
        [ -z "$port" ] && port="$(url_get_query_param "$url" "mport")"
        [ -z "$port" ] && port="443"
        password="$(url_decode "$(url_get_userinfo "$url")")"
        obfuscator_type=$(url_get_query_param "$url" "obfs")
        obfuscator_password=$(url_get_query_param "$url" "obfs-password")
        [ -z "$obfuscator_password" ] && obfuscator_password=$(url_get_query_param "$url" "obfs_password")
        upload_mbps=$(url_get_query_param "$url" "upmbps")
        [ -z "$upload_mbps" ] && upload_mbps=$(url_get_query_param "$url" "up_mbps")
        download_mbps=$(url_get_query_param "$url" "downmbps")
        [ -z "$download_mbps" ] && download_mbps=$(url_get_query_param "$url" "down_mbps")
    ...
    vmess)
        ...
        # (vmess://uuid@host:port?params#remark) is handled as a fallback if the
        # base64 decode yields no JSON.
        vmess_json="$(vmess_link_to_json "$raw_url")"
        if [ -n "$vmess_json" ] && printf '%s' "$vmess_json" | jq -e 'type == "object"' >/dev/null 2>&1; then
            vm_server=$(printf '%s' "$vmess_json" | jq -r '.add // "" | tostring')
            vm_port=$(printf '%s' "$vmess_json" | jq -r '.port // "" | tostring')
            vm_uuid=$(printf '%s' "$vmess_json" | jq -r '.id // "" | tostring')
            vm_security=$(printf '%s' "$vmess_json" | jq -r '.scy // "auto" | tostring')
            vm_alter_id=$(printf '%s' "$vmess_json" | jq -r '.aid // 0 | tostring')
            vm_net=$(printf '%s' "$vmess_json" | jq -r '.net // "tcp" | tostring')
            vm_host=$(printf '%s' "$vmess_json" | jq -r '.host // "" | tostring')
            vm_path=$(printf '%s' "$vmess_json" | jq -r '.path // "" | tostring')
            vm_tls=$(printf '%s' "$vmess_json" | jq -r '.tls // "" | tostring')
            vm_sni=$(printf '%s' "$vmess_json" | jq -r '.sni // "" | tostring')
            vm_alpn=$(printf '%s' "$vmess_json" | jq -r '.alpn // "" | tostring')
            vm_fp=$(printf '%s' "$vmess_json" | jq -r '.fp // "" | tostring')
        ...
        # Fallback format: vmess:// standard URL form (vmess://uuid@host:port?...)
        vm_server=$(url_get_host "$url")
        vm_port=$(url_get_port "$url")
        vm_uuid=$(url_get_userinfo "$url")
        vm_security=$(url_get_query_param "$url" "encryption")
        vm_alter_id=$(url_get_query_param "$url" "alterId")
    ...
    tls | reality)
        ...
        config=$(
            sing_box_cm_set_tls_for_outbound \
                ...
                "$outbound_tag" \
                "$sni" \
                "$({ [ "$insecure" = "1" ] || [ "$insecure" = "true" ]; } && echo true)" \
...
sing_box_cf_prepare_subscription_batch() {
    ...
    printf '%s' "$config" | jq -c \
        ...
        # Statically reject outbounds the current sing-box build cannot load, and normalize Hysteria2.
        | [ $candidates[]
            | . as $ob
            | (($ob.remark // $ob.tag // "") | tostring) as $name
            | if ($ob.type == "shadowsocks" and ($ob.tls.enabled == true)) then
                ...
                ($ob
                 ...
                 | del(.utls)
                 | del(.transport)
                 | if .tls then .tls.enabled = true else .tls = {enabled: true} end
                 | if (.tls.server_name == null or .tls.server_name == "") then .tls.server_name = (.server // "") else . end) as $norm_ob
                | {ob: $norm_ob, name: $name}
              else
                {ob: $ob, name: $name}
              end
          ] as $kept

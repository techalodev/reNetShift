...
sing_box_cm_add_hysteria2_outbound() {
    ...
    echo "$config" | jq \
        ...
        + (if $is_multi_port then {
                ...
                server_port: (if ($server_port == "" or $server_port == null) then 443 else ($server_port | tonumber) end)
            } end)
        + (if $obfuscator_type != "" and $obfuscator_type != "none" and $obfuscator_password != "" then {
            obfs: {
                type: $obfuscator_type,
                password: $obfuscator_password
            }
        } else {} end)
        + (if $upload_mbps != "" and $upload_mbps != null then {up_mbps: ($upload_mbps | tonumber)} else {} end)
        + (if $download_mbps != "" and $download_mbps != null then {down_mbps: ($download_mbps | tonumber)} else {} end)
        + (if $network != "" and $network != null then {network: $network} else {} end)
    )]'
}

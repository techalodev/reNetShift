...
function createSectionContent(section) {
  ...
  o = section.taboption(
    "subscription",
    form.Button,
    "_update_subscription_btn",
    _("Manual update"),
    _("Download and apply the latest proxy configurations from the configured subscription URLs immediately"),
  );
  o.inputtitle = _("Update subscription now");
  o.inputstyle = "apply";
  o.depends({ connection_type: "proxy", proxy_config_type: "subscription" });
  o.onclick = function (ev, section_id) {
    return fs.exec("/usr/bin/netshift", ["subscription_update"]).then(function (res) {
      if (res.code === 0) {
        ui.addNotification(null, E("p", {}, _("Subscription updated successfully")), "info");
      } else {
        ui.addNotification(null, E("p", {}, _("Failed to update subscription: %s").format(res.stderr || res.stdout || "")), "error");
      }
    }).catch(function (err) {
      ui.addNotification(null, E("p", {}, _("Failed to update subscription: %s").format(err.message || err)), "error");
    });
  };

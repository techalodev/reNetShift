...
function validateHysteria2Url(url) {
  try {
    ...
    if (queryString) {
      ...
      if (paramsKeys.includes("insecure") && !["0", "1", "true", "false", ""].includes(params.insecure.toLowerCase()))
        return {
          valid: false,
          message: _("Invalid HY2 URL: insecure must be 0 or 1")
        };
      if (paramsKeys.includes("allowInsecure") && !["0", "1", "true", "false", ""].includes(
      ...
      if (paramsKeys.includes("obfs") && params.obfs !== "none" && !params["obfs-password"] && !params["obfs_password"])
        return {
          valid: false,
          message: _(
...
var NetShiftShellMethods = {
  ...
  getClashApiConnections: async () => callBaseMethod(
    NetShift.AvailableMethods.CLASH_API,
    [NetShift.AvailableClashAPIMethods.GET_CONNECTIONS]
  ),
  closeClashApiConnection: async (id) => callBaseMethod(NetShift.AvailableMethods.CLASH_API, [
    NetShift.AvailableClashAPIMethods.CLOSE_CONNECTION,
    id
  ]),
  closeClashApiAllConnections: async () => callBaseMethod(NetShift.AvailableMethods.CLASH_API, [
  ...
  subscriptionUpdate: async (section) => {
    const response = await executeShellCommand({
      command: "/usr/bin/netshift",
      args: section ? ["subscription_update", section] : ["subscription_update"],
      timeout: 12e4
    });
    if (response.code === 0 || response.stdout && !response.stdout.includes("\u274C")) {
      return {
        success: true,
        data: response.stdout
      };
    ...
    return {
      success: false,
      message: response.stderr || response.stdout || "Subscription update failed"
    };
  },
...
function matchesProxyCode(proxyKey, targetCode) {
...
function findProxy(proxies, targetCode) {
  return proxies.find(
    (p) => matchesProxyCode(p.code, targetCode) || p.value?.name && matchesProxyCode(p.value.name, targetCode)
  );
}
async function getDashboardSections() {
  ...
  const allMeta = metaRes && metaRes.success ? metaRes.data : {};
  ...
  const data = configSections.filter(
    ...
    if (section.connection_type === "proxy") {
      ...
      if (section.proxy_config_type === "urltest") {
        ...
        const outbounds = (outbound?.value?.all ?? []).map((code) => findProxy(proxies, code)).filter(Boolean).map((item, index) => ({
          code: item?.code || "",
          displayName: getProxyUrlName(section.urltest_proxy_links?.[index]) || item?.value?.name || decodeURIComponent(item?.code || ""),
      ...
      if (section.proxy_config_type === "subscription") {
        ...
        const selectorOutbounds = (selector?.value?.all ?? []).flatMap(
          (code) => {
            ...
            const isLegacyFastest = matchesProxyCode(item.code, sectionName + "-urltest-out") || item.value?.name?.toLowerCase() === "urltest" || item.value?.type?.toLowerCase() === "urltest";
            return [
              {
                code: item.code,
                displayName: isLegacyFastest ? _("Fastest") : item?.value?.name || decodeURIComponent(item.code),
                latency: item?.value?.history?.[0]?.delay || 0,
                type: item?.value?.type || "",
                selected: matchesProxyCode(selector?.value?.now || "", item.code)
              }
        ...
        if (outbounds.length === 0 && fallbackUrltest) {
          const fallbackOutbounds = (fallbackUrltest?.value?.all ?? []).map((code) => findProxy(proxies, code)).filter(Boolean).map((item) => ({
            code: item.code,
        ...
        if (outbounds.length === 0) {
          const systemProxies = /* @__PURE__ */ new Set([
            "DIRECT",
            "GLOBAL",
            "REJECT",
            "dns-out",
          ...
          const discovered = proxies.filter(
...
function renderLoaderCircleIcon24(size = 16) {
...
function prettyBytes(n) {
  ...
  if (n === void 0 || n === null || isNaN(Number(n)) || Number(n) <= 0) {
    return "0 B";
  }
  const num = Number(n);
  if (num < 1e3) {
  ...
  const formatted = Number((num / Math.pow(1e3, exponent)).toPrecision(3));
...
function renderDefaultState({
  ...
  latencyFetching,
  activePingTags: activePingTags2,
  ...
  function renderSubscriptionInfo() {
    ...
    if (hasTraffic) {
      ...
      const barColor = percent > 90 ? "var(--color-danger, #e74c3c)" : percent > 75 ? "var(--color-warning, #f39c12)" : "var(--color-success, #2ecc71)";
      elements.push(
        E(
          "div",
          {
          ...
          [
            E(
              "div",
              {
                style: "display: flex; justify-content: space-between; margin-bottom: 4px; color: var(--text-color, #ccc); font-size: 12px;"
              },
              [
                E(
                  "span",
                ...
                hasExpire ? (() => {
                  const days = Math.ceil(
                    ((meta.expire || 0) * 1e3 - Date.now()) / (1e3 * 60 * 60 * 24)
                  );
                  ...
                  return E(
                    "span",
                    {
                      style: days <= 3 ? "color: var(--color-danger, #e74c3c); font-weight: bold;" : ""
                    },
            ...
            E(
              "div",
              {
                style: "height: 4px; background: rgba(255,255,255,0.12); border-radius: 2px; overflow: hidden;"
              },
              [
                E("div", {
      ...
      const dateStr = new Date((meta.expire || 0) * 1e3).toLocaleDateString();
      elements.push(
        E(
          "div",
          {
            class: "pdk_subscription-info__expire",
            style: "font-size: 12px; margin-top: 4px; color: var(--text-color, #ccc);"
          },
          [
            E(
              "span",
    ...
    return E(
      "div",
      {
        ...
        style: "padding: 0 4px 8px 4px; margin-bottom: 8px; border-bottom: 1px solid rgba(255,255,255,0.06);"
      },
      elements
    );
  }
  function renderOutbound(outbound) {
    const isPinging = Boolean(activePingTags2?.has(outbound.code));
    ...
    const latencyContent = isPinging ? E("span", { class: "pdk_dashboard-page__outbound-grid__item__latency-loading" }, [
...
var activePingTags = /* @__PURE__ */ new Set();
var subscriptionUpdatingSections = /* @__PURE__ */ new Set();
var sectionPingProgress = /* @__PURE__ */ new Map();
...
async function handleTestSingleProxyLatency(tag) {
...
async function handleUpdateSubscription(section) {
  ...
  await renderSectionsWidget();
  try {
    const res = await NetShiftShellMethods.subscriptionUpdate();
    await fetchDashboardSections();
...
async function runDnsCheck() {
  ...
  const { state: state2, description } = getMeta({ atLeastOneGood, allGood });
  updateCheckStore({
    order,
    code,
    title,
...
function netshiftCard(systemInfo, check) {
  ...
  if (status === "outdated") {
    actions.push({
      ...
      kind: "self_update",
      text: latest ? _("Install %s").replace("%s", latest) : _("Update reNetShift"),
...
// src/netshift/tabs/connections/connections.store.ts
var initialConnectionsStore = {
  connections: [],
  downloadTotal: 0,
  uploadTotal: 0,
  memory: 0,
...
// src/netshift/tabs/connections/renderConnections.ts
function formatDuration(startTime) {
  if (!startTime) return "0s";
  const start = new Date(startTime).getTime();
  const diffSec = Math.max(0, Math.floor((Date.now() - start) / 1e3));
...
function renderConnections({
  ...
  const { connections, downloadTotal, uploadTotal, memory, hostnames, searchQuery, filterNetwork, isPaused, closingIds, closingAll, loading } = state2;
  ...
  return E("div", { class: "pdk_connections-page" }, [
    ...
    E("div", { class: "pdk_connections-table-wrapper" }, [
      filtered.length === 0 ? E(
        "div",
        { style: "padding: 32px; text-align: center; color: var(--text-muted, #888);" },
        ...
        E(
          ...
          filtered.map((conn) => {
            ...
            const hostDisplay = conn.metadata.host ? `${conn.metadata.host}:${conn.metadata.destinationPort}` : `${conn.metadata.destinationIP}:${conn.metadata.destinationPort}`;
            ...
            const clientContent = deviceName ? E("div", { style: "display: flex; flex-direction: column; gap: 2px;" }, [
...
// src/netshift/tabs/connections/initController.ts
var state = { ...initialConnectionsStore };
var pollInterval = null;
var hostnamesInterval = null;
function render4() {
  ...
  const content = renderConnections({
    ...
    onFilterNetworkChange: (filterNetwork) => {
...
function initController4() {
  state = { ...initialConnectionsStore, closingIds: /* @__PURE__ */ new Set() };
  ...
  hostnamesInterval = window.setInterval(updateHostnames, 15e3);
  ...
  function startPolling() {
    ...
    fetchSnapshot();
    pollInterval = window.setInterval(fetchSnapshot, 3e3);
  }
  connectWs();
}
...
// src/netshift/tabs/connections/styles.ts
var CONNECTIONS_STYLES = `
...
// src/netshift/tabs/connections/index.ts
var ConnectionsTab = {
  initController: initController4,
  render: renderConnectionsTab,
  styles: CONNECTIONS_STYLES
};

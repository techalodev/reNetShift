...
export const NetShiftShellMethods = {
  ...
  getClashApiConnections: async () =>
    callBaseMethod<NetShift.GetClashApiConnections>(
      NetShift.AvailableMethods.CLASH_API,
      [NetShift.AvailableClashAPIMethods.GET_CONNECTIONS],
    ),
  closeClashApiConnection: async (id: string) =>
    callBaseMethod<{ success: boolean }>(NetShift.AvailableMethods.CLASH_API, [
      NetShift.AvailableClashAPIMethods.CLOSE_CONNECTION,
      id,
    ]),
  closeClashApiAllConnections: async () =>
    callBaseMethod<{ success: boolean }>(NetShift.AvailableMethods.CLASH_API, [
      NetShift.AvailableClashAPIMethods.CLOSE_ALL_CONNECTIONS,
    ]),
  getSubscriptionMetadata: async (section?: string) =>
    callBaseMethod<Record<string, NetShift.SubscriptionMetadata>>(
      NetShift.AvailableMethods.GET_SUBSCRIPTION_METADATA,
      section ? [section] : [],
    ),
  restart: async () =>
  ...
  subscriptionUpdate: async (section?: string) => {
    const response = await executeShellCommand({
      command: '/usr/bin/netshift',
      args: section
        ? ['subscription_update', section]
        : ['subscription_update'],
      timeout: 120000,
    });
    ...
    if (
      response.code === 0 ||
      (response.stdout && !response.stdout.includes('❌'))
    ) {
      return {
        success: true,
    ...
    return {
      success: false,
      message:
        response.stderr || response.stdout || 'Subscription update failed',
    };
  },

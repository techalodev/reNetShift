...
<p align="center">
  <img src="./docs/icon.png" alt="reNetShift" width="128" />
  <br>
  <br>
  <a href="https://github.com/techalodev/reNetShift/releases">
    <img src="https://img.shields.io/github/release/techalodev/reNetShift/all.svg">
  </a>
</p>
...
**reNetShift** - маршрутизатор трафика для OpenWrt. Направляйте нужные ресурсы в туннель, а остальное - напрямую. Открытое ПО на базе [sing-box](https://github.com/SagerNet/sing-box).

---

## Функции

- [x] **Поддержка всех современных протоколов** - AmneziaWG (AWG), WireGuard, Hysteria2, VLESS, VMess, Shadowsocks, Trojan, SOCKS.
- [x] **Информация о подписке** - остаток трафика, использованный объем и срок окончания подписки на дашборде.
- [x] **Мониторинг соединений (Connections Viewer)** - живой мониторинг активных сессий, поиск по IP/хостам и сброс соединений в реальном времени.
...
- [x] **Самообновление из веб-интерфейса** - проверка и установка обновлений прямо из LuCI<br><sub>асинхронно · бэкап конфига · без риска «окирпичивания»</sub>
- [x] **Веб-интерфейс LuCI** - дашборд, менеджер компонентов, диагностика и настройки без ручной правки конфигов
- [x] **IPv6, блокировка DoH, глобальный прокси** - полная маршрутизация v6 через туннель, защита DNS роутера, режим «весь трафик в туннель»

---

## Установка reNetShift

Для установки и обновления достаточно одной команды:

```sh
sh <(wget -O - https://raw.githubusercontent.com/techalodev/reNetShift/main/install.sh)
```

Интерфейс появится в LuCI: **Services → reNetShift**.
...
- **Install extended** - расширенное ядро (даёт клиентский транспорт **xhttp**, только клиентский режим). Также поддерживается **AmneziaWG (AWG)**, **WireGuard** и **VMess**.
